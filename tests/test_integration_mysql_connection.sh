...
echo "Running MySQL connection integration test..."
python3 -m pytest tests/test_integration_mysql_connection.py::test_actual_mysql_connection -v -s --capture=no --tb=short

# Capture the exit code of pytest
PYTEST_EXIT_CODE=$?

if [ $PYTEST_EXIT_CODE -eq 0 ]; then
  echo "MySQL connection integration test PASSED."
else
  echo "MySQL connection integration test FAILED (Exit code: $PYTEST_EXIT_CODE)."
fi
